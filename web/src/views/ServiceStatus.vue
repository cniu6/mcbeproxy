        ...
        <n-collapse-item v-for="s in servers" :key="s.id" :name="s.id">
          <template #header>
            <n-space align="center" wrap size="small">
              ...
              <n-tag v-if="shouldShowServerLatencyOverview(s)" size="small" :type="getLatencyType(s)">{{ getLatencyText(s) }}</n-tag>
...
const loadData = async () => {
  ...
  try {
    ...
    if (payload && payload.success && payload.data) {
      ...
      const newHistory = payload.data.latency_history || {}
      Object.keys(serverLatencyHistory).forEach(k => {
        if (!newHistory[k]) delete serverLatencyHistory[k]
      })
