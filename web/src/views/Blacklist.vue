<template>
  <div>
    ...
    <n-card>
      <n-data-table :columns="columns" :data="blacklist" :bordered="false" :pagination="{ pageSize: 20 }" :scroll-x="700" />
    </n-card>
...
const openExportModal = () => { exportJson.value = JSON.stringify(blacklist.value, null, 2); showExportModal.value = true }
const copyExport = async () => { await navigator.clipboard.writeText(exportJson.value); message.success('已复制') }
const downloadExport = () => {
  const blob = new Blob([exportJson.value], { type: 'application/json' })
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'blacklist.json'; a.click()
}
